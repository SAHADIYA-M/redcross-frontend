...
import { api, NeedCluster, EvidenceItem, FusionCandidate, formatReportTime, formatTimeOnly } from './api';
import { useClusters, useFusionCandidates, useClusterDetail } from './hooks'
...
function Overview({
  ...
  const pendingDuplicates = (candidates || []).filter(
    (c) => c.status === 'PENDING' && c.type === 'POSSIBLE_DUPLICATE'
  ).length;
  const pendingConflicts = (candidates || []).filter(
    (c) => c.status === 'PENDING' && c.type === 'POSSIBLE_CONFLICT'
  ).length;
  const awaitingVerification = clusters.filter((c) => c.status === 'REVIEW').length;
  const hasAttentionItems = pendingDuplicates > 0 || pendingConflicts > 0 || awaitingVerification > 0;

  const handleResolveAttention = async () => {
    try {
      setResolvingAttention(true);
      await api.resolveAllFusion('MERGED');
      if (onRefresh) await onRefresh();
    } catch (e: any) {
      alert(e.message || 'Failed to resolve attention items');
    } finally {
  ...
  return (
    <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto w-full">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-sm text-gray-400">
            ...
            <span className="text-gray-200">·</span>
            <span>Last processed: {lastUpdatedTime || 'Just now'}</span>
          </div>
        </div>
        {onRefresh && (
          <button
            onClick={onRefresh}
            className="flex items-center gap-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 bg-white border border-gray-200 px-3 py-1.5 rounded-lg shadow-2xs hover:bg-gray-50 transition-colors"
          >
            <span>↻</span> Refresh Picture
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {loading ? (
          <div className="col-span-4 text-center py-10 text-gray-400">Loading metrics...</div>
        ) : (
          [
            {
              value: clusters.reduce((acc, c) => acc + c.observations, 0).toString(),
              label: 'Observations',
              sub: 'Raw inputs received',
            },
            {
              value: clusters.length.toString(),
              label: 'Need Clusters',
              sub: 'Consolidated situations',
            },
            {
            ...
            {
              value: clusters.reduce((acc, c) => acc + c.conflicts, 0).toString(),
              label: 'Conflicts',
              sub: 'Evidence disagrees',
            },
          ].map((m) => (
          ...
          {clusters.map((c) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-gray-400">
                ...
                {c.status === 'REVIEW' && (
                  <span className="text-red-500 font-medium">Needs verification</span>
                )}
                {c.status === 'VERIFIED' && (
          ...
          <div>
            ...
            {hasAttentionItems ? (
              <div className="border border-amber-200 bg-amber-50 rounded-xl px-5 py-4 space-y-3">
                {pendingDuplicates > 0 && (
                  <div className="flex items-center justify-between gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 text-sm text-amber-800 hover:text-amber-900 text-left font-medium flex-1 min-w-0"
                    >
                      <span className="text-amber-500 shrink-0 font-bold">⚠</span>
                      <span className="truncate">
                        {pendingDuplicates} possible duplicate {pendingDuplicates === 1 ? 'group' : 'groups'}
                      </span>
                    </button>
                    <button
                      onClick={() => setPage('fusion')}
                      className="text-xs px-2 py-0.5 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded font-medium transition-colors shrink-0 border border-amber-300"
                    >
                      Review
                    </button>
                  </div>
                ...
                {pendingConflicts > 0 && (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      <span className="text-amber-500 shrink-0 font-bold">⚠</span>
                      <span className="truncate">
                        {pendingConflicts} conflicting {pendingConflicts === 1 ? 'observation' : 'observations'}
                      </span>
                    </button>
                    <button
                      onClick={() => setPage('fusion')}
                ...
                {awaitingVerification > 0 && (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      <span className="text-amber-500 shrink-0 font-bold">⚠</span>
                      <span className="truncate">
                        {awaitingVerification} {awaitingVerification === 1 ? 'cluster' : 'clusters'} awaiting verification
                      </span>
                    </button>
                    <button
                      onClick={() => setPage('clusters')}
                ...
                {(pendingDuplicates > 0 || pendingConflicts > 0) && (
                  <div className="pt-2 border-t border-amber-200/80">
                    <button
                      onClick={handleResolveAttention}
                      disabled={resolvingAttention}
                      className="w-full text-xs font-semibold py-1.5 px-3 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors flex items-center justify-center gap-1.5 shadow-2xs disabled:opacity-50"
                    >
                      <span>✓</span>
                      <span>{resolvingAttention ? 'Resolving...' : 'Rectify & Resolve Flags'}</span>
                    </button>
                  </div>
                )}
              </div>
            ) : (
              <div className="border border-emerald-200 bg-emerald-50 rounded-xl px-4 py-3.5 text-sm text-emerald-800 flex items-center gap-2.5">
                <span className="text-emerald-600 font-bold shrink-0">✓</span>
                <span>Operational picture verified — no pending duplicates or conflicts.</span>
              </div>
            )}
          </div>
  ...
  const handleDismissAll = async () => {
    try {
      ...
      for (const item of pending) {
        await api.resolveFusionCandidate(item.id, 'DISMISSED');
      }
      onResolved();
    } catch (e: any) {
      alert(e.message || 'Failed to dismiss candidates');
    } finally {
      setResolving(null);
    }
  };
        ...
        <div className="flex items-center gap-2">
          {pending.length > 0 && (
            <button
              ...
              className="text-xs font-medium text-gray-600 hover:text-gray-900 bg-white border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors shadow-2xs disabled:opacity-50"
            >
              {resolving === 'all' ? 'Dismissing...' : `Dismiss All (${pending.length})`}
            </button>
          )}
...
function SubmitReportModal({ onClose, onSubmitted }: { onClose: () => void; onSubmitted?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const now = new Date();
      const clientTime = now.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                onClick={async () => {
                  ...
                  if (confirm(`Are you sure you want to delete report ${report.id}?`)) {
                    try {
                      ...
                      setReport(null);
                      setError(`Report ${report.id} was deleted successfully.`);
                    } catch (err: any) {
                      alert(err.message || 'Failed to delete report');
                    }
                  }
                }}
                className="text-xs font-medium text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 px-3 py-1 rounded-lg transition-colors"
              >
                Delete Report
              </button>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <div>
                <div className="text-xs text-gray-500 mb-1">Reported Time</div>
                <div className="font-medium text-gray-900">{formatReportTime(report.timestamp)}</div>
                <div className="text-[11px] text-gray-400 mt-0.5">Local browser time</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={async () => {
              if (confirm(`Are you sure you want to delete Cluster ${cluster.id}? This will remove it from the operational picture.`)) {
                try {
            ...
            className="text-xs font-medium text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded-lg transition-colors"
          >
            Delete Cluster
          </button>
          ...
          <div className="border border-[#E4E7EC] bg-white rounded-xl px-6 py-5">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-[#E4E7EC] pt-4">
              {[
                ...
                { label: 'First reported', value: formatReportTime(cluster.firstSeen) || 'N/A' },
                { label: 'Latest update', value: formatReportTime(cluster.lastUpdate) || 'N/A' },
              ].map(item => (
                <div key={item.label}>
                ...
                {evidence.map(e => (
                  <div key={e.id} className={`border rounded-xl px-4 py-3.5 ${e.role === 'conflicts' ? 'border-amber-200 bg-amber-50' : 'border-[#E4E7EC] bg-white'}`}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-gray-400">{formatTimeOnly(e.time)}</span>
                    </div>
                ...
                {timeline.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="relative mb-5 last:mb-0">
                      ...
                      <div className="text-xs font-mono text-gray-400 mb-0.5">{formatTimeOnly(item.time)}</div>
      ...
      <div className="flex flex-col items-center gap-5 min-w-[360px]">
        ...
        <div className="flex flex-wrap items-start justify-center gap-3 w-full">
          {supporters.map((e, idx) => (
            <div key={`${e.id}-${idx}`} className="flex flex-col items-center gap-1.5">
              ...
              <div className="border border-emerald-200 bg-emerald-50/70 rounded-lg px-3 py-2 text-center">
                ...
                <div className="text-xs text-gray-500 mt-0.5">{formatTimeOnly(e.time)}</div>
              </div>
            </div>
          ))}
          {conflicts.map((e, idx) => (
...
export default function App() {
  ...
  const { clusters: apiClusters, loading, refetch: refetchClusters } = useClusters();
  ...
  const refreshAll = async () => {
    await Promise.all([refetchClusters(), refetchFusion()]);
    setLastUpdatedTime(new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true }));
  };

  const handleResetData = async () => {
    if (confirm('Reset all demo data back to clean operational state? This will clear any test reports or duplicate clusters.')) {
      try {
        await api.resetData();
        await refreshAll();
      } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]" style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}>
      ...
      <header className="bg-white border-b border-[#E4E7EC] sticky top-0 z-40">
        <div className="px-6 lg:px-10 max-w-[1400px] mx-auto h-[52px] flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="text-right hidden lg:block">
              ...
              <div className="text-[10px] text-gray-400 mt-0.5">Last updated {lastUpdatedTime}</div>
            </div>
            <button
              onClick={handleResetData}
              title="Reset all demo data back to clean operational state"
              className="hidden md:inline-flex items-center text-xs text-gray-500 hover:text-gray-800 bg-white border border-[#E4E7EC] px-2.5 py-1.5 rounded-lg hover:bg-gray-50 transition-colors font-medium whitespace-nowrap"
            >
              ↻ Reset Data
            </button>
            <button
        ...
        {mobileNavOpen && (
          <div className="sm:hidden border-t border-[#E4E7EC] bg-[#FAFAFA] px-6 py-3 space-y-1">
            ...
            <button
              onClick={() => { handleResetData(); setMobileNavOpen(false) }}
              className="w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium text-gray-500 hover:bg-gray-100"
            >
              ↻ Reset Demo Data
            </button>
          </div>
      ...
      <main className="overflow-x-hidden">
        ...
        {page === 'fusion' && <FusionQueue candidates={fusionCandidates} loading={fusionLoading} error={fusionError} onResolved={refreshAll} />}
      ...
      {showImport && (
        <ImportModal onClose={() => setShowImport(false)} onProcess={() => { refreshAll(); setPage('overview'); }} />
      )}
      {showSubmitReport && (
        <SubmitReportModal onClose={() => setShowSubmitReport(false)} onSubmitted={refreshAll} />
      )}
    </div>
  )
}
